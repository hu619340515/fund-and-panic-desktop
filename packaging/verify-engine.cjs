...
server.listen(0, '127.0.0.1', () => {
  ...
  server.close(async () => {
    const child = spawn(executable, ['--host','127.0.0.1','--port',String(port),'--database',join(directory,'data/panic-index.db'),
      ...
      '--client-version','2.0.1','--instance-id','build-verification'], {windowsHide:true, stdio:['ignore','pipe','pipe'], env:{...process.env,PYTHONHOME:'',PYTHONPATH:''}})
    let stderr = ''
    ...
    try {
      ...
      console.log('内置 EXE 健康检查通过：客户端 2.0.1 / 引擎 3.0-realtime / 数据库 5，中文路径可用')
