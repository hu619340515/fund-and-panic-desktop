...
async function main() {
  const executablePath = path.resolve(process.argv[2] ?? path.join(root, 'desktop/dist/FundAndPanic-Portable-2.0.4-x64.exe'))
  ...
  try {
    ...
    assert.equal(report.health.client_version, '2.0.4')
